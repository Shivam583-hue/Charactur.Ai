...
const Companions = ({ data }: CompanionsProps) => {
  ...
  return (
    <div className='grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg: gird-cols-5 xl:grid-cols-6 gap-2 pb-10'>
      {data.map((item) => (
          ...
          <Link href={`/chat/${item.id}`}>
            <CardHeader className='flex items-center justify-center text-center text-muted-foreground'>
              <div className='relative w-32 h-32'>
                <Image fill className='rounded-xl object-cover' alt={item.name} src={item.src} />
              </div>
